...
try { 
    var result = api_instance.createRegistrationSignin(registrationId, idempotencyKey);
    print(result);
} catch (e) {
    print("Exception when calling SigninsApi->createRegistrationSignin: $e\n");
}
```
...
try { 
    ...
    print("Exception when calling SigninsApi->createRegistrationSignout: $e\n");
}
```
