...
final signinCreateParams = SigninCreateParams(); // SigninCreateParams | Params for creating a Signin can omit certain fields if a `registration_id` is present.

try { 
    final result = api_instance.createSignin(signinCreateParams);
    print(result);
} catch (e) {
...
final api_instance = SigninsApi();
final signinId = signinId_example; // String | 
final signinUpdateParams = SigninUpdateParams(); // SigninUpdateParams | The only updatable values for a `Signin` are `badge_number`, `badge_returned`, `is_accounted_for`, `is_signed_out`, and `is_acknowledged`.  `is_signed_out` and `is_acknowledged` are pseudo attributes that once set to true, are irreversible.
...
try { 
    final result = api_instance.updateSignin(signinId, signinUpdateParams, idempotencyKey);
    print(result);
} catch (e) {
